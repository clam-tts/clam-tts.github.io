...
function divBuilderLibriSpeech(id, data) {
  ...
  for (let i = 0; i < data["text_list"].length; i++) {
    ...
    // inject values
    const promptText = copiedNode.querySelector(".clam-prompt-text > span");
    promptText.innerText = data["prompt_text_list"][i];
    const text = copiedNode.querySelector(".clam-text > span");
    text.innerText = data["text_list"][i];

    const audioOursAll = copiedNode.querySelector(".clam-audioviz audio");
    ...
    // inject functions
    const canvas = copiedNode.querySelector(".clam-audioviz canvas");
    audioVisualizer(audioOursAll, canvas, data["prompt_time"][i]);

    const toggle = copiedNode.querySelector(".clam-toggle");
    const sampleBox = copiedNode.querySelector(".clam-sample-box");
    ...
    const openToggle = copiedNode.querySelector(
      ".clam-toggle div:nth-child(1)",
    );
    const closeToggle = copiedNode.querySelector(
      ".clam-toggle div:nth-child(2)",
    );
...
function divBuilderMLS(id, data) {
  ...
  const cards = document.querySelectorAll(id + " .clam-card");
  for (let i = 0; i < data["text_list"].length; i++) {
    ...
    cards[i]
      .querySelectorAll(".clam-sample-box table tr")
      .forEach((elm, idx) => {
...
document
  .querySelector('button[data-bs-toggle="tab"][data-bs-target="#vctk-box"]')
  .addEventListener("shown.bs.tab", function (event) {
    if (!vctkFlag) {
      vctkFlag = true;
      divBuilderVCTK("#vctk-box", vctkData);
    }
  });
document
